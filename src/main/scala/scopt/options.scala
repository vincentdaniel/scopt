...
object Read {
  ...
  implicit val stringRead: Read[String] = reads {
    identity
  }
  implicit val doubleRead: Read[Double] = reads {
    _.toDouble
  }
  ...
  implicit val bigDecimalRead: Read[BigDecimal] = reads {
    BigDecimal(_)
  }
  ...
  implicit val fileRead: Read[File] = reads {
    new File(_)
  }
  implicit val uriRead: Read[URI] = reads {
    new URI(_)
  }
  implicit val inetAddress: Read[InetAddress] = reads {
...
abstract case class OptionParser[C](programName: String) {
  ...
  def showTryHelp(): Unit = {
    ...
    Console.err.println("Try " + oxford(helpOptions.toList map {
      _.fullName
  ...
  def usage: String = {
    ...
    while (unseen.nonEmpty) {
      for {
        ...
        val cs = unseen filter {
          _.getParentId.contains(x.id)
        }
        unseen --= cs
  ...
  private[scopt] def commandExample(cmd: Option[OptionDef[_, C]]): String = {
    ...
    else if (as.nonEmpty) text ++= as map {
      _.argName
    }
    text.mkString(" ")
  }
  ...
  def parse(args: Seq[String], init: C): Option[C] = {
    var i = 0
    val pendingOptions = ListBuffer() ++ (nonArgs filterNot {
      _.hasParent
    })
    val pendingArgs = ListBuffer() ++ (arguments filterNot {
      _.hasParent
    })
    val pendingCommands = ListBuffer() ++ (commands filterNot {
      _.hasParent
    })
    val occurrences = collection.mutable.ListMap[OptionDef[_, C], Int]().withDefaultValue(0)
    var _config: C = init
    var _error = false
    ...
    def pushChildren(opt: OptionDef[_, C]): Unit = {
      ...
      pendingOptions insertAll(0, nonArgs filter { x => x.getParentId.contains(opt.id) &&
        !pendingOptions.contains(x)
      })
      pendingArgs insertAll(0, arguments filter { x => x.getParentId.contains(opt.id) &&
        !pendingArgs.contains(x)
      })
      pendingCommands insertAll(0, commands filter { x => x.getParentId.contains(opt.id) &&
    ...
    def handleShortOptions(g0: String): Unit = {
      val gs = (0 until g0.length) map { n => g0.substring(0, g0.length - n) }
      gs flatMap { g => pendingOptions map {
        (g, _)
      }
      } find { case (g, opt) =>
    ...
    def handleChecks(c: C): Unit = {
      Validation.validateValue(checks flatMap {
        _.checks
      })(c) match {
